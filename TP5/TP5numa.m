...
chi2inv(0.95, (5-1) * (2-1))

% The chi2 is inferior too P(chi2_0<X)=95 therfore we cannot say that the
% two sample don't follow the same law.
...
global_mean = (length(Re_01) * m_01 + length(Re_02) * m_02 + length(Re_04) * m_04 + length(Re_06) * m_06) / n_value
global_var = (length(Re_01) * var_02 + length(Re_02) * var_04 + length(Re_04) * var_01 + length(Re_06) * var_06) / n_value;
